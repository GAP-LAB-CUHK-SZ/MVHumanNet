...
FEMALE_COCO_PATH = "model_coco_plus/female_smpl_with_cocoplus_reg.pkl"
...
class SMPL():
    def __init__(self, sex, model_dir):
        ...
        model_paths = {
            ...
            'female_coco_plus': os.path.join(model_dir, FEMALE_COCO_PATH),
        }

        with open(model_paths[sex], 'rb') as f:
            smpl_model = pickle.load(f, encoding='latin1')

        ## people snap shot用到了自己额外优化过的model params （selfRecon)
        if sex == 'female_coco_plus':
            self.J_regressor = np.array(smpl_model['J_regressor']).T
            self.weights = np.array(smpl_model['weights'])
            self.posedirs = np.array(smpl_model['posedirs'])
            self.v_template = np.array(smpl_model['v_template'])
            self.shapedirs = np.array(smpl_model['shapedirs'])
            self.faces = np.array(smpl_model['f']).astype('int32')
            self.kintree_table = np.array(smpl_model['kintree_table']).astype('int64')
        else:
    ...
    def __call__(self, pose, beta):
        ...
        ## 用J_regressor和vertices得到T-pose下的joints坐标
