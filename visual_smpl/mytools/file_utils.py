...
def read_annot_save(annotname, mode='body25'):
...
def read_annot(annotname, mode='body25'):
    ...
    # data[0]["bbox_handl2d"][:4] = list(np.array(data[0]["bbox_handl2d"][:4])*2)
    
    for i in range(len(data)):
        
        '''resize camera'''
        data[i]["bbox"][:4] = list(np.array(data[i]["bbox"][:4])*2)
        data[i]["bbox_handl2d"][:4] = list(np.array(data[i]["bbox_handl2d"][:4])*2)
        data[i]["bbox_handr2d"][:4] = list(np.array(data[i]["bbox_handr2d"][:4])*2)
        data[i]["bbox_face2d"][:4] = list(np.array(data[i]["bbox_face2d"][:4])*2)
        
        data_keypoints = np.array(data[i]["keypoints"])
        data_keypoints[:,:2] = data_keypoints[:,:2]*2
        data[i]["keypoints"] = (data_keypoints)
        
        data_handl2d = np.array(data[i]["handl2d"])
        data_handl2d[:,:2] = data_handl2d[:,:2]*2
        data[i]["handl2d"] = (data_handl2d)
        
        data_handr2d = np.array(data[i]["handr2d"])
        data_handr2d[:,:2] = data_handr2d[:,:2]*2
        data[i]["handr2d"] = (data_handr2d)
        
        data_face2d = np.array(data[i]["face2d"])
        data_face2d[:,:2] = data_face2d[:,:2]*2
        data[i]["face2d"] = (data_face2d)
